...
- `PixelboxRuntime.md`: Phaser compatibility-layer details, including LDtk-backed map loading, typed-LDtk-entity to legacy-sprite normalization, sidecar compat metadata merging for LDtk round-trips, compatibility map virtualization, fixed-aspect integer scaling behavior, bitmap-font text rendering (`setCharset` support), dialog wrapping behavior, and automation hooks.
- `Audio.md`: Manifest format and runtime audio mapping.
- `MapConverter.md`: Legacy Pixelbox -> LDtk migration workflow, including official LDtk 1.5.3 project generation, typed LDtk entity definition emission, level-layout preservation during regeneration, `maps.ldtk.meta.json` sidecar extraction, real tileset dimension extraction, compat metadata packing, entity field serialization, and runtime-preserved logical map names.
